...
func UpdatesolnetworkpackageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.UpdateSolNetworkPackageOutput
...
func CreateUpdatesolnetworkpackageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_sol_nsd_v1_ns_descriptors_nsdInfoId",
		mcp.WithDescription("<p>Updates the operational state of a network package.</p> <p>A network package is a .zip file in CSAR (Cloud Service Archive) format defines the function packages you want to deploy and the Amazon Web Services infrastructure you want to deploy them on.</p> <p>A network service descriptor is a .yaml file in a network package that uses the TOSCA standard to describe the network functions you want to deploy and the Amazon Web Services infrastructure you want to deploy the network functions on.</p>"),
		mcp.WithString("nsdInfoId", mcp.Required(), mcp.Description("ID of the network service descriptor in the network package.")),
		mcp.WithString("nsdOperationalState", mcp.Required(), mcp.Description("Input parameter: Operational state of the network service descriptor in the network package.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatesolnetworkpackageHandler(cfg),
	}
}
