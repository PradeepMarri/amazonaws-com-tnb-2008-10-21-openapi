...
func ListsolfunctionpackagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sol/vnfpkgm/v1/vnf_packages%s", cfg.BaseURL, queryString)
		...
		var result models.ListSolFunctionPackagesOutput
...
func CreateListsolfunctionpackagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_sol_vnfpkgm_v1_vnf_packages",
		mcp.WithDescription("<p>Lists information about function packages.</p> <p>A function package is a .zip file in CSAR (Cloud Service Archive) format that contains a network function (an ETSI standard telecommunication application) and function package descriptor that uses the TOSCA standard to describe how the network functions should run on your network.</p>"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListsolfunctionpackagesHandler(cfg),
	}
}
