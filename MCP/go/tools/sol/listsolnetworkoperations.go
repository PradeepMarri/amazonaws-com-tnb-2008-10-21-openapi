...
func ListsolnetworkoperationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sol/nslcm/v1/ns_lcm_op_occs%s", cfg.BaseURL, queryString)
		...
		var result models.ListSolNetworkOperationsOutput
...
func CreateListsolnetworkoperationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_sol_nslcm_v1_ns_lcm_op_occs",
		mcp.WithDescription("<p>Lists details for a network operation, including when the operation started and the status of the operation.</p> <p>A network operation is any operation that is done to your network, such as network instance instantiation or termination.</p>"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListsolnetworkoperationsHandler(cfg),
	}
}
