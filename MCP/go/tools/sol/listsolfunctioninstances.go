...
func ListsolfunctioninstancesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sol/vnflcm/v1/vnf_instances%s", cfg.BaseURL, queryString)
		...
		var result models.ListSolFunctionInstancesOutput
...
func CreateListsolfunctioninstancesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_sol_vnflcm_v1_vnf_instances",
		mcp.WithDescription("<p>Lists network function instances.</p> <p>A network function instance is a function in a function package .</p>"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListsolfunctioninstancesHandler(cfg),
	}
}
